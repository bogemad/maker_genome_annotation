...
sub construct_training_set{
	...
	$perlCmdString="perl $PASAHOME/seqclean/seqclean/seqclean transcripts.fasta -v $PASAHOME/UniVec 1>seqclean.stdout 2>seqclean.stderr";
	...
	while(<CONFIG>){
	    ...
	    s/<__MIN_PERCENT_ALIGNED__>/80/;
	    s/<__MIN_AVG_PER_ID__>/90/;
	    print TEMP;
	}
	close(CONFIG);
	close(TEMP);
	...
	my $gmapoption = "";
	$gmapoption = "--ALIGNERS gmap" if ($useGMAPforPASA);
	$gmapoption = "--ALIGNERS blat" if (! $useGMAPforPASA);
	...
	$perlCmdString="perl $PASAHOME/scripts/pasa_asmbls_to_training_set.dbi --pasa_transcripts_fasta $pasaDBname.assemblies.fasta --pasa_transcripts_gff3 $pasaDBname.pasa_assemblies.gff3 1>pasa_asmbls_to_training_set.stdout 2>pasa_asmbls_to_training_set.stderr";
	...
	$cmdString = "grep complete ../pasa/$pasaDBname.assemblies.fasta.transdecoder.pep".' | perl -pe \'s/>(\S+).*/$1\$/\'';
    ...
    open(FILE, "../pasa/$pasaDBname.assemblies.fasta.transdecoder.genome.gff3") or die("\nCould not open ../pasa/$pasaDBname.assemblies.fasta.transdecoder.genome.gff3\n");
    my $sum=0;
    my $n=0;
    while(<FILE>){
	if(/\tgene\t/){
...
sub alignments_and_hints{
    ...
    # pslCDnaFilter
    $cmdString="pslCDnaFilter -minId=0.8 -localNearBest=0.005 -ignoreNs -bestOverlap "
